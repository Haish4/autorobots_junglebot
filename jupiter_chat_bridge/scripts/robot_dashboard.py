...
class SpeechBox(QFrame):
    def __init__(self, accent_color, placeholder, parent=None):
        ...
        self._label.setStyleSheet(f"color: {PALETTE['text_secondary']}; font-size: 13px; font-family: {SANS}; background: transparent; border: none;")
        layout.addWidget(self._label)

    def set_text(self, text):
        self._label.setText(text)
        self._label.setStyleSheet(f"color: {PALETTE['text_primary']}; font-size: 13px; font-family: {SANS}; background: transparent; border: none;")
...
class TopBar(QFrame):
    def __init__(self, parent=None):
        ...
        title.setStyleSheet(f"color: {PALETTE['text_primary']}; font-size: 16px; font-family: {MONO}; font-weight: bold; letter-spacing: 4px; background: transparent; border: none;")

        subtitle = QLabel("SYSTEM OPERATIONS MONITOR  //  ROS INTERFACE")
        subtitle.setStyleSheet(f"color: {PALETTE['text_muted']}; font-size: 9px; font-family: {MONO}; letter-spacing: 2px; background: transparent; border: none;")
        ...
        self._clock.setStyleSheet(f"color: {PALETTE['text_primary']}; font-size: 16px; font-family: {MONO}; background: transparent; border: none;")

        self._date = QLabel()
        self._date.setAlignment(Qt.AlignRight)
        self._date.setStyleSheet(f"color: {PALETTE['text_muted']}; font-size: 9px; font-family: {MONO}; letter-spacing: 1px; background: transparent; border: none;")
...
class CameraPanel(QFrame):
    def __init__(self, parent=None):
        ...
        self._placeholder.setStyleSheet(f"color: {PALETTE['text_muted']}; font-family: {MONO}; font-size: 12px; letter-spacing: 4px; background: transparent; border: none;")
...
class SnapshotPanel(QFrame):
    """Shows the last frame sent to the vision model, with a flash effect."""
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        self._placeholder = QLabel("AWAITING CAPTURE")
        self._placeholder.setAlignment(Qt.AlignCenter)
        self._placeholder.setStyleSheet(f"color: {PALETTE['text_muted']}; font-family: {MONO}; font-size: 10px; letter-spacing: 3px; background: transparent; border: none;")
    ...
    def update_snapshot(self, pixmap):
        if self._placeholder.isVisible():
            self._placeholder.hide()
            self._img_label.show()

        scaled = pixmap.scaled(self.width(), self.height() - 4, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        self._img_label.setPixmap(scaled)

        # Flash border blue to indicate new capture
        self.setStyleSheet(f"""
            QFrame {{
                background-color: {PALETTE['bg_card']};
        ...
        """)
        QTimer.singleShot(800, self._reset_border)
...
class RobotDashboard(QWidget):
    ...
    def __init__(self):
        ...
        left_col.addWidget(self._snapshot_panel)
        ...
        self._row_snapshots  = InfoRow("VISION TRIGGERS")

        for row in [self._row_uptime, self._row_frames, self._row_utterances, self._row_replies, self._row_snapshots]:
        ...
        self._speech_box = SpeechBox(accent_color=PALETTE["accent_blue"], placeholder="Listening for operator input...")
        ...
        self._reply_box = SpeechBox(accent_color=PALETTE["accent_green"], placeholder="Awaiting bridge node...")
        ...
        rospy.Subscriber("/vision_snapshot",    Image,  self.snapshot_callback)
    ...
    def camera_callback(self, data):
        try:
            ...
            self._camera_panel.update_frame(QPixmap.fromImage(qt_img))
            self._last_msg_time["camera"] = time.time()
            self._frame_count += 1
            self._node_cam.set_status("ok")
        except Exception as e:
            rospy.logerr("[Dashboard] Camera frame error: %s", e)
    ...
    def snapshot_callback(self, data):
        try:
            ...
            h, w, ch = cv_img.shape
            qt_img = QImage(cv_img.data, w, h, ch * w, QImage.Format_RGB888)
            self._snapshot_panel.update_snapshot(QPixmap.fromImage(qt_img))
            self._snapshot_count += 1
        except Exception as e:
            rospy.logerr("[Dashboard] Snapshot error: %s", e)
    ...
    def _update_telemetry(self):
        ...
        self._row_snapshots.set_value(str(self._snapshot_count))
